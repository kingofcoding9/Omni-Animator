...
export default function App() {
  ...
  const handleAddFreeformLayer = (points: Point[], center: Point) => {
    dispatch({
      ...
      payload: { points, center, currentFrame }
    });
  };
  ...
  const handleExportSpriteSheet = async () => {
    ...
    const confirmSheet = window.confirm(
      `SPRITE SHEET EXPORTER:\n\nThis will combine ${totalFrames} animation frames into a single, combined image grid (sheet). Perfect for web/game engine integration.\n\nDo you want to proceed?`
    );
    ...
    try {
      ...
      alert(err.message || "Failed to render full spritesheet.");
    } finally {
      setIsLoading(false);
    }
  };
